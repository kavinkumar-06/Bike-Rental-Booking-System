...
const AdminDashboard = () => {
  ...
  const fetchBikes = async () => {
    setLoading(true);
    try {
      const res = await axios.get(`${API_BASE}/api/admin/bikes`, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isUpdateMode) {
        await axios.put(
          `${API_BASE}/api/admin/bikes/${currentBikeId}`,
          formData,
          {
            headers: { Authorization: `Bearer ${user.token}` },
          }
        ...
        await axios.post(`${API_BASE}/api/admin/bikes`, formData, {
          headers: { Authorization: `Bearer ${user.token}` },
        });
  ...
  const handleDelete = async (bikeId) => {
    try {
      await axios.delete(`${API_BASE}/api/admin/bikes/${bikeId}`, {
        headers: { Authorization: `Bearer ${user.token}` },
      });
