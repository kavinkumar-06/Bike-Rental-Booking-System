...
const Bookings = () => {
  ...
  useEffect(() => {
    const fetchBookings = async () => {
      ...
      try {
        ...
        const response = await axios.get(`${API_BASE}/api/bookings`, config);
  ...
  if (loading) {
    return (
      <div className="text-center pt-24 p-4">Loading your bookings...</div>
    );
  }
              ...
              <div className="flex justify-between items-start mb-4">
                <div>
                  <h2 className="text-2xl font-bold text-blue-600">
                    {booking.bikeId?.name || "Bike Details Missing"}
                  </h2>
                  <p className="text-gray-500">{booking.bikeId?.type}</p>
                </div>
                <span
                  className={`font-semibold text-lg ${getStatusClasses(
                    booking.paymentStatus
                  )}`}
                >
                  {booking.paymentStatus?.charAt(0).toUpperCase() +
                    booking.paymentStatus?.slice(1) || "N/A"}
                </span>
              </div>
