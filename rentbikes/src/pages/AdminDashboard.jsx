...
const AdminDashboard = () => {
  ...
  const fetchBookings = async () => {
    ...
    try {
      const res = await axios.get(
        "https://bike-rental-booking-system-3.onrender.com/api/admin/bookings",
        {
          headers: { Authorization: `Bearer ${user.token}` },
        },
      );
  ...
  const fetchUsers = async () => {
    ...
    try {
      const res = await axios.get(
        "https://bike-rental-booking-system-3.onrender.com/api/admin/users",
        {
          headers: { Authorization: `Bearer ${user.token}` },
        },
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isUpdateMode) {
        await axios.put(
          `https://bike-rental-booking-system-3.onrender.com/api/admin/bikes/${currentBikeId}`,
          formData,
          {
            headers: { Authorization: `Bearer ${user.token}` },
          },
      ...
      setToastData({
        message: "Failed to submit. Are you logged in as an admin?",
        type: "error",
      });
    }
  };
  ...
  const handleDelete = async (bikeId) => {
    try {
      await axios.delete(
        `https://bike-rental-booking-system-3.onrender.com/api/admin/bikes/${bikeId}`,
        {
          headers: { Authorization: `Bearer ${user.token}` },
        },
      );
  ...
  const handleUpdateBooking = async (bookingId, newStatus) => {
    try {
      await axios.put(
        `https://bike-rental-booking-system-3.onrender.com/api/admin/bookings/${bookingId}/status`,
        ...
        return (
          <div className="max-w-lg mx-auto bg-white p-8 rounded-lg shadow-md">
            <h2 className="text-2xl font-bold mb-4">
              {isUpdateMode ? "Update Bike" : "Add a New Bike"}
            </h2>
          ...
          <div className="flex flex-col gap-4">
            {bookings.map((booking) => (
              <div key={booking._id} className="border rounded-lg p-4 shadow">
                ...
                <p>
                  User:{" "}
                  {booking.userId?.name || booking.userId?.username || "N/A"}
                </p>
                ...
                <div className="flex justify-end space-x-2 mt-4">
                  <button
                    onClick={() => handleUpdateBooking(booking._id, "paid")}
  ...
  return (
    <div className="container mx-auto p-4 pt-24">
      ...
      <div className="flex justify-center space-x-4 mb-8">
        <button
          onClick={() => setSelectedFeature("bikes")}
          className={`px-4 py-2 rounded ${selectedFeature === "bikes" ? "bg-blue-500 text-white" : "bg-gray-200"}`}
        >
          Manage Bikes
        </button>
        <button
          onClick={() => setSelectedFeature("add-bike")}
          className={`px-4 py-2 rounded ${selectedFeature === "add-bike" ? "bg-blue-500 text-white" : "bg-gray-200"}`}
        >
          Add New Bike
        </button>
        <button
          onClick={() => setSelectedFeature("bookings")}
          className={`px-4 py-2 rounded ${selectedFeature === "bookings" ? "bg-blue-500 text-white" : "bg-gray-200"}`}
        >
          View Bookings
        </button>
        <button
          onClick={() => setSelectedFeature("users")}
          className={`px-4 py-2 rounded ${selectedFeature === "users" ? "bg-blue-500 text-white" : "bg-gray-200"}`}
        >
          View All Users
        </button>
      </div>
