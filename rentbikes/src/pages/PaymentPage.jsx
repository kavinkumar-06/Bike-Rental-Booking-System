...
const PaymentPage = () => {
  ...
  const handleRazorpayPayment = async () => {
    ...
    try {
      ...
      const orderResponse = await axios.post(
        "https://bike-rental-booking-system-3.onrender.com/api/payment/create-order",
        {
        ...
        handler: async (response) => {
          try {
            const updateResponse = await axios.post(
              "https://bike-rental-booking-system-3.onrender.com/api/bookings/update-payment",
              {
  ...
  if (!booking) {
    return (
      <div className="text-center pt-24 p-4">Loading booking details...</div>
    );
  }
        ...
        <div className="bg-white rounded-lg shadow-md p-8">
          ...
          <div className="mb-4">
            <p className="text-gray-700">
              <strong>Bike:</strong> {booking.bikeId?.name || "N/A"} (
              {booking.bikeId?.type || "N/A"})
            </p>
            <p className="text-gray-700">
              <strong>From:</strong>{" "}
            ...
            <p className="text-gray-700">
              <strong>To:</strong> {new Date(booking.toDate).toLocaleString()}
            </p>
            <p className="text-gray-700">
              <strong>Hourly Rate:</strong> ₹
              {booking.bikeId?.rentalPricePerHour || "N/A"} / hour
            </p>
            <p className="text-gray-700">
          ...
          <button
            ...
            className={`w-full text-white font-bold py-3 px-4 rounded-full transition duration-300 ${
              paymentLoading || totalCost <= 0 || !user
